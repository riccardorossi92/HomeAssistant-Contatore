"""Sensori diagnostici Ireti: i dati veri finiscono in external statistics
(statistics.py). Questi servono solo a vedere a colpo d'occhio lo stato
dell'import, stessa filosofia minimale di pcf_common/edistribuzione/areti
(un dispositivo "account" più uno per POD, poche entità diagnostiche)."""
...
def _device_info_account(entry: ConfigEntry) -> DeviceInfo:
    ...
    stesso identico pattern di pcf_common/edistribuzione/areti."""
    return DeviceInfo(
...
def build_ireti_entities(hass, coordinator: IretiCoordinator) -> list[SensorEntity]:
    """Costruisce le entità sensor per una config entry Ireti - chiamata
    ...
    entities: list[SensorEntity] = [IretiPodConfiguratiSensor(coordinator, entry)]
    for pod in coordinator.pods:
        entities.append(IretiUltimaDataDisponibileSensor(coordinator, entry, pod, id_padre))
        entities.append(IretiConsumoGiornoSensor(coordinator, entry, pod, id_padre))
    return entities


class IretiPodConfiguratiSensor(SensorEntity):
    """Vive sul dispositivo "account", equivalente di
    Pcf/Edistribuzione/AretiPodConfiguratiSensor."""

    _attr_has_entity_name = True
    _attr_entity_category = EntityCategory.DIAGNOSTIC
    _attr_icon = "mdi:counter"

    def __init__(self, coordinator: IretiCoordinator, entry: ConfigEntry) -> None:
        super().__init__()
...
class IretiUltimaDataDisponibileSensor(
    CoordinatorEntity[IretiCoordinator], RestoreEntity, SensorEntity
):
    ...
    def __init__(
        self, coordinator: IretiCoordinator, entry: ConfigEntry, pod: str,
...
class IretiConsumoGiornoSensor(CoordinatorEntity[IretiCoordinator], RestoreEntity, SensorEntity):
    """Consumo totale (kWh) dell'ultimo giorno importato per un POD -
    equivalente di EdistribuzioneConsumoGiornoSensor. Volutamente senza
    ...
    def extra_state_attributes(self):
        dati_pod = (self.coordinator.data or {}).get("by_pod", {}).get(self._pod, {})
        return {"giorno": dati_pod.get("ultimo_giorno_importato")}
