...
E-Distribuzione e' oggi solo un segnaposto (nessuna API nota ancora, quindi
async_setup_entry non fa nulla di funzionale per quel ramo).
"""
...
def _trova_coordinator(hass: HomeAssistant, entry_id: str | None) -> PcfCoordinator:
    """Individua il coordinator PCF su cui agire.

    Con una sola istanza "pcf" configurata entry_id e' superfluo; con piu'
    istanze va indicato. Le eventuali entry E-Distribuzione (senza
    coordinator PCF) vengono ignorate qui.
    """
    coordinators: dict = {
        eid: c for eid, c in hass.data.get(DOMAIN, {}).items() if isinstance(c, PcfCoordinator)
    }
    if not coordinators:
        raise HomeAssistantError("Nessuna istanza Duereti/Unareti configurata in contatore_letture.")

    if entry_id:
        if entry_id not in coordinators:
            raise HomeAssistantError(
                f"Nessuna istanza PCF con entry_id '{entry_id}'. Disponibili: {list(coordinators)}"
            )
        return coordinators[entry_id]

    if len(coordinators) > 1:
        raise HomeAssistantError(
            "Ci sono più istanze Duereti/Unareti configurate: specifica 'entry_id'. "
...
async def _async_registra_servizi(hass: HomeAssistant) -> None:
    """Registra le azioni dell'integrazione (una sola volta, condivise tra
    tutti i distributori di tipo 'pcf')."""
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if info["kind"] == "pcf":
        pcf_rimasti = any(isinstance(c, PcfCoordinator) for c in hass.data[DOMAIN].values())
        if not pcf_rimasti:
